...
class AdminProductGridController extends ModuleAdminController
{
    ...
    public function __construct()
    {
        ...
        JOIN `' . _DB_PREFIX_ . 'product_shop` sa ON (a.`id_product` = sa.`id_product` AND sa.id_shop = ' . pSQL($id_shop) . ')
        ...
        $this->_select .= 'a.`reference`, sa.`active` AS `active`, shop.`name` AS `shopname`, a.`id_product` as `id`,
...
//            }
    ...
    public function ajaxProcessSaveMore()
    {
        ...
        if (!call_user_func(array('Validate', Product::$definition['fields']['wholesale_price']['validate']), $wholesale_price)) {
...
//					$unit_price = $product->price / $unit_price;
    ...
    public function updateObjectField($class_name, $field, $id, $value)
    {
        ...
        if (!$multi_shop_active || ($lang && $multi_lang_shop) || Shop::getContext() == Shop::CONTEXT_ALL || $fix_only_one) {
    ...
    public function getLinkProductForList($echo)
    {
        if (version_compare(_PS_VERSION_, '1.7.0.0', '<')) {
            ...
            $url = $this->getPreviewUrl1516($product);
        } else {
            $url = $this->getPreviewUrl17($echo);
        }
        $this->context->smarty->assign(array(
    ...
    public function getPreviewUrl1516(Product $product)
    {
        ...
        if (!$product->active) {
            ...
            $preview_url .= ((strpos($preview_url, '?') === false) ? '?' : '&') . 'adtoken=' . $token . '&ad=' . $admin_dir . '&id_employee=' . (int)$this->context->employee->id;
        }
     ...
     * @return bool|string
     */
    public function getPreviewUrl17($productId)
    {
        $kernel = ${'GLOBALS'}['kernel'];

        $container = $kernel->getContainer();
        $wrapper = new \PrestaShop\PrestaShop\Adapter\Product\AdminProductWrapper(
            $container->get('translator'),
            $container->get('prestashop.adapter.legacy.context')
        );
        return $wrapper->getPreviewUrlFromId($productId);
    }
