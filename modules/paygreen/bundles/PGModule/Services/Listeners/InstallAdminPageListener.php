...
class PGModuleServicesListenersInstallAdminPageListener
{
    ...
    {
        $this->logger->info("Install Paygreen admin pages.");

        try {
            $this->adminMenuHandler->insertBackoffice();
        } catch (Exception $exception) {
            $this->logger->error("Error during Paygreen admin page installation : " . $exception->getMessage(), $exception);
        }
    }

    public function removeAdminPage()
    {
        $this->logger->info("Uninstall Paygreen admin page.");
