        ...
        $id_customer = 0, $use_customer_price = true, $id_cart = 0, $real_quantity = 0, $id_customization = 0)
    {
        ...
        if (($only_reduc || $use_reduc) && $specific_price) {
            ...
            if (isset($specific_price['reduction'])) {
                $specific_price['reduction'] = strval($specific_price['reduction']);
            }
        }
        if ($use_reduc) {
    ...
    */
    public static function getProductProperties($id_lang, $row, Context $context = null)
    {
        ...
        if (isset($row['id_product_attribute'])) {
            $configs_onsale_show_discounts = $groupinc->getGIConfigurations($id_shop, $id_product, $id_customer, $id_country, $id_state, $id_currency, $id_lang, true, true, $row['id_product_attribute'], 0, false, true);
            if (!empty($configs_onsale_show_discounts)) {
                $properties['on_sale'] = 1;
                return $properties;
            }
            $configs_onsale = $groupinc->getGIConfigurations($id_shop, $id_product, $id_customer, $id_country, $id_state, $id_currency, $id_lang, true, false, $row['id_product_attribute'], 0, false, true);
            if (!empty($configs_onsale)) {
                $properties['on_sale'] = 1;
                return $properties;
            }
        }
